...
class RMQsubscriber:
    ...
    def setup(self):
        ...
        channel.exchange_declare(exchange=self.exchange, exchange_type='direct') # topic (Prod) direct (Testing)
...
queueName1 = 'reportsedc'
key1 = "reportsedc" # reports.create.cef (Prod)  reportsedc (testing)
notification_consumer_config1 = {'host': 'fishymq.xlab.si',
                                'port': 45672,
                                'exchange' : "reportsedc", # tasks (Prod) reportsedc (testing)
                                'login':'tubs',
                                'password':'sbut'}
